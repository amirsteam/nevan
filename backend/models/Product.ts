...
export interface IProductMethods {
    reduceStock(items: { variantId?: string | Types.ObjectId; quantity: number }[]): Promise<void>;
...
// Statics
productSchema.statics.search = function (this: IProductModel, query: string, options: any = {}) {
...
// Instance methods
productSchema.methods.reduceStock = async function (this: IProduct, items: { variantId?: string | Types.ObjectId; quantity: number }[]) {
    for (const item of items) {
        if (item.variantId) {
            const variant = (this.variants as any).id(item.variantId);
            if (variant) {
...
productSchema.methods.getVariant = function (this: IProduct, variantId: string | Types.ObjectId) {
    return (this.variants as any).id(variantId);
};

productSchema.methods.findVariant = function (this: IProduct, size: string, color: string) {
