 ...
 */
const createOrder = async (userId: string, orderData: OrderData): Promise<IOrder> => {
 ...
 */
const calculateShippingCost = (address: ShippingAddress, subtotal: number): number => {
 ...
 * Get order details
 */
const getOrderById = async (orderId: string, userId: string | null = null): Promise<IOrder> => {
...
const cancelOrder = async (
    ...
    for (const item of (order as any).items) {
        ...
        if (product) {
            ...
            (product as any).soldCount = Math.max(0, (product as any).soldCount - item.quantity);
            await product.save();
        }
    }
 ...
 */
const getAllOrders = async (options: GetOrdersOptions = {}): Promise<OrdersResult> => {
