 ...
 */
const getProducts = async (options: ProductsOptions = {}): Promise<ProductsResult> => {
    const {
        page = 1,
        limit = 12,
        category,
 ...
 */
const addProductImages = async (productId: string, files: MulterFile[]): Promise<IProduct> => {
    const product = await Product.findById(productId);
    if (!product) {
        throw new AppError('Product not found', 404);
    }
 ...
 */
const deleteProductImage = async (productId: string, imageId: string): Promise<IProduct> => {
    const product = await Product.findById(productId);
    if (!product) {
        throw new AppError('Product not found', 404);
    }
