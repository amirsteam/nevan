...
const protect = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        if (typeof (user as any).changedPasswordAfter === 'function' && (user as any).changedPasswordAfter(decoded.iat)) {
 ...
 */
const optionalAuth = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    let token: string | undefined;
